...
#define HAS_NORMAL 
#define LIGHTING_MODEL SCNLightingModelPhysicallyBased
...
// MARK: Converting between coordinate systems
...
/* This version of `scn_importanceSampleGGX_brdf` assumes that
 *    N = (0, 0, 1)
 * (i.e we work in tangent space)
 */
static float3 scn_importanceSampleGGX_brdf(float2 random, float roughness) __attribute__((overloadable)) {
    ...
    // importance sampling (spherical coordinates)
    //   inversion method for the PDF p = D(h)<n⋅h>
    float cosTheta = sqrt((1.0 - random.x) / (1.0 + (alpha * alpha - 1.0) * random.x + 1e-6));
    float phi = 2.0 * M_PI_F * random.y;
    
    // importance sampling (cartesian coordinates)
...
/* This version of `scn_importanceSampleCosine_brdf` assumes that
 *    N = (0, 0, 1)
 * (i.e we work in tangent space)
 */
static float3 scn_importanceSampleCosine_brdf(float2 random) __attribute__((overloadable)) {
    ...
    // importance sampling (spherical coordinates)
    //   Malley's method: uniform sampling on a disk, then projection on the hemisphere
    ...
    float x = r * cosPhi;
    float y = r * sinPhi;
    float z = sqrt(max(0.f, 1.f - random.x)); // sqrt(1 - x^2 + z^2) = sqrt(1 - r^2)
    return float3(x, y, z);
}
...
// from https://twvideo01.ubm-us.net/o1/vault/gdc2017/Presentations/Hammon_Earl_PBR_Diffuse_Lighting.pdf
                              ...
                              float roughness)
{
    ...
    /* Everything is computed in tangent space
     *
     *   1. V = (Vt, Vb, Vn) = (sinTheta * cosPhi, sinTheta * sinPhi, cosTheta)
     *   2. N⋅V = Vn = VoN
     *
     * We make the assumption that the shape of the BRDF is isotropic, so let's pick phi = 0 (any phi works)
     *
     *  1. Vn = cosTheta = NoV
     *  2. Vt = sinTheta = sqrt(1 - cosTheta^2)     | because cos(phi) = 1
     *  3. Vb = 0                                   | because sin(phi) = 0
     */
    ...
    for (ushort i = 0; i < sampleCount; ++i) {
        ...
        // Specular
        float3 h = scn_importanceSampleGGX_brdf(random, roughness); // half-vector
        float3 l = reflect(-v, h); // lighting vector (reflect view vector around half-vector)
        
        float NoL = saturate(l.z); //               | because N = (0, 0, 1)
        float NoH = saturate(h.z); //               | because N = (0, 0, 1)
        ...
        // Diffuse
        float3 Ld = scn_importanceSampleCosine_brdf(random); // Diffuse light direction
        float NoLd = saturate(Ld.z);
        if (NoLd > 0) {
            float3 Hd = normalize(v + Ld);
            float NoHd = Hd.z;
            // float LdotHd = saturate(dot(Ld, Hd));
                                  ...
                                  float3                             n_cube)
{
    ...
    for (ushort i = 0; i < sampleCount; ++i) {
        float2 random = scn_sampleHammersley(i, sampleCount);
        float3 l = scn_importanceSampleCosine_brdf(random, n); // lighting vector

        float NoL = saturate(dot(n, l));
        
        if (NoL > 0) {
            float3 Li = environment.sample(linearSampler, float3(l.x , l.z, -l.y), level(environmentMipmapLevel)).rgb;
            L += Li; //                             | should be 1/π * Li * N⋅L but the p.d.f for cosinus-weighted importance sampling is 1/π * N⋅L
            weight += 1.0;
        }
    }
...
// no light squareRadius, because no windowing when it's not deferred
                                      ...
                                      float                              environmentIntensity)
{
    ...
    for (ushort i = 0; i < sampleCountDiffuse; ++i) {
        ...
        if (NoL > 0) {
            ...
            irradiance += Li; //                       | should be 1/π * Li * N⋅L but the p.d.f for cosinus-weighted importance sampling is 1/π * N⋅L
        }
    }
    ...
    float alpha = correctedRoughness * correctedRoughness; // perceptually-linear roughness
    ...
    for (ushort i = 0; i < sampleCountSpecular; ++i) {
        ...
        float3 h = scn_importanceSampleGGX_brdf(random, correctedRoughness, n); // half-vector
        float3 l = reflect(-v, h); // lighting vector (reflect view vector around half-vector)
        ...
        if (NoH * NoV > 0) {
            ...
            if (pdf >= 0) {
                float3 l = D * F * G / (4.0f * NoV); // N⋅L simplifies
                specular += Li * l / pdf;
                specularWeight += 1.0f;
            }
#else
...
vertex commonprofile_io commonprofile_vertAE25ABD6B84BE17507AB45D8BCEE03AD975A1E98DA6BE0D962E52A83F9C71DE8(
#if !defined(USE_TESSELLATION)
...
fragment SCNOutput commonprofile_fragAE25ABD6B84BE17507AB45D8BCEE03AD975A1E98DA6BE0D962E52A83F9C71DE8(commonprofile_io                 in                               [[ stage_in  ]]
